 ...
 * 把 scripts/_new_word_entries.json（600題新題庫稽核出的缺漏單字）upsert 進 Supabase words 表。
...
const entries = JSON.parse(fs.readFileSync(path.join(__dirname, '_new_word_entries.json'), 'utf8'));
...
async function main() {
  ...
  const { count } = await supabase
    ...
    .contains('tags', ['question_bank_2026_07']);
  console.log(`[驗證] Supabase question_bank_2026_07 標籤共 ${count} 字`);
}
