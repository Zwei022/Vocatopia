...
const FC_SWIPE_COMMIT_DEG    = 10;    // 放開時旋轉角度超過這個值就判定滑動成立

// 注意：這裡刻意不用 Pointer Events。Capacitor iOS 用的 WKWebView 對
// pointermove/pointerup 的支援不穩定（實機測試會整個收不到後續事件，
// 滑動完全沒反應），改用瀏覽器最原始、支援度最沒有疑慮的 touch 事件
// （桌面測試則另外綁 mouse 事件），兩套各自獨立、互不影響。
function _fcSwipeInit() {
  ...
  // ── touch（手機/平板，含 Capacitor WKWebView）──
  wrap.addEventListener('touchstart', e => {
    if (e.target.closest('.fc-rich-btn, .spk-btn, .fcs-icon-btn')) return;
    const t = e.touches[0];
    startDrag(t.clientX, t.clientY);
  }, { passive: true });
  wrap.addEventListener('touchmove', e => {
    if (!dragging) return;
    e.preventDefault(); // touch-action:none 已經擋掉原生捲動，這裡再保險一次
    const t = e.touches[0];
    dx = t.clientX - startX;
    dy = t.clientY - startY;
    applyMove();
  }, { passive: false });
  wrap.addEventListener('touchend', () => { if (dragging) finishDrag(); });
  wrap.addEventListener('touchcancel', () => { if (dragging) finishDrag(); });
...
const FEATURE_HINTS = {
  ...
  tetrisSolo: {
    ...
    points: [
      ...
      '消行得分：1／2／3／4 行分別 100／300／500／800 分，按下加速鍵可瞬間落底（硬降）每格 +2 分',
      '消行會觸發單字選擇快問（限時 7 秒）：答對 +50 分且連續答對有加乘（最高 ×2），答錯 -30 分並中斷連勝',
    ],
  },
  tetrisRanked: {
    title: '積分模式',
    points: [
      ...
      '消行得分：1／2／3／4 行分別 100／300／500／800 分，按下加速鍵可瞬間落底（硬降）每格 +2 分',
      '消行會觸發單字選擇快問（限時 7 秒）：答對 +50 分且連續答對有加乘（最高 ×2），答錯 -30 分並中斷連勝',
      '每 60 秒會出一題英文選擇題（限時 30 秒）：答對 +150 分，答錯不扣分但會鎖底層一整行',
      '每累積 5000 分會遇到一次閱讀理解關卡（限時 40 秒）：答對 +300 分，答錯不扣分但左右兩排會被封鎖，填滿整排才能解鎖',
    ],
  },
};
